...
public abstract class Shape {
    ...
    */
    ...
    protected float[] genAxes() {
        ...
        for(int i = 2; i < n; i += 2)
            getAxis(axes, i, verts[i-2], verts[i-1], verts[i], verts[i+1]);
        // Finally we add in the axis connecting the first and last vertices.
        getAxis(axes, 0, verts[0], verts[1], verts[n], verts[n+1]);
        return axes;
    }
    ...
    protected void getProjection(float[] dest, int offset, float x, float y) {
        ...
        float min = x*verts[0] + y*verts[1];
        float max = min;
        float p;
        
        for(int i = 2; i < verts.length; i += 2) {
            p = x*verts[i] + y*verts[i+1];
            if(p < min)
                min = p;
            else if(p > max)
                max = p;
     ...
     * dest.length -1 }.
     */
    protected static void getAxis(float[] dest, int offset, float x1, float y1,
            float x2, float y2) {
     ...
     */
    protected static boolean projsIntersect(float min1, float max1,
     ...
     */
    protected static boolean projContains(float min1, float max1,
            float min2, float max2) {
     ...
     */
    protected static boolean projContainsPoint(float min, float max, float p) {
        return min <= p && max >= p;
    }
