 ...
 * {@code Sendable} is an easy way to accidentally let a bug slip in, and so
 * here we are. (In the current iteration we still suffer from essentially the
 * same problem but are now additionally burdened with the interim step of
 * writing to a DataCompound before saving, so was anything really gained?)
 * 
 * @see Sendable
 */
